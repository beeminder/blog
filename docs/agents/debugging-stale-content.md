...
Runbook for: "I edited a pad on <etherpad-host>, Render deployed, but the change isn't appearing on blog.beeminder.com."
...
```bash
curl -s "https://<etherpad-host>/<pad>/export/txt" | grep -F "<a distinctive string you just typed>"
```
...
# 1. add $SENTINEL to <etherpad-host>/<pad>, save
curl -s "https://<etherpad-host>/<pad>/export/txt" | grep -F "$SENTINEL"     # confirm pad has it
