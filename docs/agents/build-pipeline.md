...
```
┌────────────────────┐          ┌──────────────────┐         ┌─────────────┐         ┌──────────────────────┐
│  padm.us (CMS)     │  fetch   │  this repo       │  push   │  Render     │  serve  │  blog.beeminder.com  │
│  Etherpad pads     │◀─────────┤  Astro build     ├────────▶│  static     ├────────▶│  (Cloudflare front)  │
│  /<pad>/export/txt │  per     │  scripts + lib   │  deploy │  site host  │  via    │                      │
└────────────────────┘  build   └──────────────────┘         └─────────────┘  CF     └──────────────────────┘
```

- **CMS** — pads live at `https://padm.us/<pad>` (a Beeminder-branded Etherpad called Beetherpad).
...
Production `ETHERPAD_DOMAIN` is `padm.us`. Only `padm.us` actually serves pad content — the other hostnames redirect or 404. If a build is succeeding, `ETHERPAD_DOMAIN` is set correctly.
...
| Var                       | Effect                                                                                                            |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `ETHERPAD_DOMAIN`         | Host to fetch pads from. Required. `padm.us` in prod, `padm.us` locally if you want real content.                 |
...
- **Body change** (typo fix, content rewrite inside an existing pad) — edit on padm.us, no commit needed. Fresh build picks it up via the live fetch.
